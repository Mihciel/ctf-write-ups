...
#pragma omp parallel for shared(data, possible_last_round_keys)
    for (size_t i = 0; i < (1L << 32); i++)
    {
        if (SUB(data[0][5], round_f(data[0][4], i, 10)) == SUB(data[0][7], round_f(data[0][6], i, 10)))
        {
#pragma omp critical
    ...
    size_t j = 1;
    while (possible_last_round_keys.size() > 1 && j < possible_last_round_keys.size())
    {
        vector<uint> tmp;
#pragma omp parallel for shared(tmp, data, possible_last_round_keys)
        for (uint k : possible_last_round_keys)
        {
            if (SUB(data[j][5], round_f(data[j][4], k, 10)) == SUB(data[j][7], round_f(data[j][6], k, 10)))
            {
#pragma omp critical
                tmp.push_back(k);
            }
        }
        possible_last_round_keys = tmp;
        j += 1;
    }
